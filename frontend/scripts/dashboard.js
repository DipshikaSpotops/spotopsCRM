$(document).ready(async function () {
  ...
  // === helpers: month normalization + Dallas date-only (GLOBAL) ===
  ...
  function dallasYMDFromISO(iso) {
    // returns "YYYY-MM-DD" interpreted in America/Chicago
    return new Date(iso).toLocaleDateString("en-CA", { timeZone: "America/Chicago" });
  }

  function todayDallasYMD() {
    // returns today's "YYYY-MM-DD" in America/Chicago
  ...
  function calculateBestSalesDay(orders) {
    const dailySales = {};
    orders.forEach(order => {
      const ymd = dallasYMDFromISO(order.orderDate);
      const value = parseFloat(order.grossProfit || 0);
  ...
  // Fetch daily orders and display in a chart
  ...
  async function fetchDailyOrders(monthShort = null, year = null) {
    ...
    // normalize month for API compatibility
  ...
  async function fetchAndDisplayThreeMonthsData() {
    ...
    try {
      ...
      for (let i = 0; i <= currentMonth; i++) {
        ...
        const response = await axios.get("https://www.spotops360.com/orders/monthly", {
          params: { month: monthStr, year: currentYear, limit: 1000 }
        });
  ...
  async function loadMonthlyCancellationRefundData(monthShort, year) {
    // normalize incoming month before calling endpoints
    monthShort = normalizeMonthToMMM(monthShort);
  ...
  document.getElementById("goToMonthBtn").addEventListener("click", async () => {
    ...
    // normalize month for API
    const monthShort = normalizeMonthToMMM(date.toLocaleString("default", { month: "short" }));
      ...
      if (Array.isArray(order.additionalInfo)) {
        order.additionalInfo.forEach(info => {
          if (info.paymentStatus === "Card charged") {
            ...
            const purchases = partPrice + shipping + others - refundedAmount;
  ...
  const handleMigrateDates = async () => {
    ...
    try {
      const response = await fetch("/admin/migrate-dates", { method: "GET" });
