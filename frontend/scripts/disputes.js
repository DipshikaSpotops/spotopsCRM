...
return (item.orderStatus === "Dispute");
});
...
const date1 = new Date(item.disputedDate);
...
const formattedDisputedDate = `${day1}${suffix(day1)} ${monthNames[date1.getUTCMonth()]}, ${year1}`;
...
${
item.orderHistory
.filter(entry => entry.includes("Order status updated to Dispute"))
.map(entry => {
...
document.getElementById("showTotalOrders").innerHTML = `Total Disputes This Month- ${allOrdersLength}`;
renderTableRows(currentPage);
createPaginationControls(Math.ceil(allOrders.length / rowsPerPage));
} catch (error) {
console.error("Error fetching current month's orders:", error);
