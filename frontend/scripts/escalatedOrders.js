// === Escalated Orders page (FULL UPDATED CODE) ===
// Changes:
// 1) Keeps the "Total Escalated Orders" count (#showTotalOrders) in sync on search, filter, paginate, and initial load.
// 2) Uses a `visibleOrders` dataset for whatever is currently shown (after search/filter).
// 3) Pagination now uses the currently visible (and table-filtered) rows count, not the full dataset.
...
$(document).ready(async function () {
  ...
  let allOrders = [];
  let visibleOrders = []; // UPDATED: dataset currently being displayed (after search/filter)
  const rowsPerPage = 25;
  let currentPage = 1;
  let lastFilteredCount = 0; // UPDATED: # rows actually rendered (post table-specific filter)
  ...
  // === RENDER & PAGINATION ===
  function renderTableRows(page, orders = visibleOrders) {
    // Check if any entry in additionalInfo has escTicked set to "Yes"
    const isEscTicked = orders.some(order =>
      order.additionalInfo && order.additionalInfo.some(info => info.escTicked === "Yes")
    );
    $("#escTickBox").prop("checked", isEscTicked);

    // Only show orders that actually have escalation ticked "Yes"
    const filteredOrders = orders.filter(order => {
      const hasEscalation = order.additionalInfo &&
        order.additionalInfo.some(info => info.escTicked === "Yes");
      return hasEscalation;
    });

    // UPDATED: keep #showTotalOrders in sync with what's rendered
    lastFilteredCount = filteredOrders.length;
    const headerEl = document.getElementById("showTotalOrders");
    if (headerEl) headerEl.textContent = `Total Escalated Orders - ${lastFilteredCount}`;
    ...
    ordersForPage.forEach((item) => {
      ...
      if (item.orderStatus === "Order Fulfilled" && item.additionalInfo.some(yard => yard.escTicked === "Yes")) {
        escalationStyle = 'style="background-color: #4c8b0f;color:white;"';
      }
      else if (item.orderStatus === "Order Cancelled" && item.additionalInfo.some(yard => yard.escTicked === "Yes")) {
        escalationStyle = 'style="background-color: #ff2121;color:white;"';
      }
      else if (item.orderStatus === "Dispute" && item.additionalInfo.some(yard => yard.escTicked === "Yes")) {
        escalationStyle = 'style="background-color: #d55900;color:white;"';
      }
      else if (item.orderStatus === "Refunded" && item.additionalInfo.some(yard => yard.escTicked === "Yes")) {
        escalationStyle = 'style="background-color: #5a3286;color:white;"';
      }
      const actions = `
    ...
    createPaginationControls(Math.ceil(lastFilteredCount / rowsPerPage));
  }
  ...
  $('#pagination-controls').on('click', '#prevPage', function () {
    if (currentPage > 1) {
      ...
      createPaginationControls(Math.ceil(lastFilteredCount / rowsPerPage)); // UPDATED
    }
  });

  $('#pagination-controls').on('click', '#nextPage', function () {
    const totalPages = Math.ceil(lastFilteredCount / rowsPerPage); // UPDATED
  ...
  try {
    ...
    // UPDATED: set visibleOrders to allOrders initially
    visibleOrders = allOrders.slice();

    // (Optional) pre-label; renderTableRows will set it accurately after filtering for escTicked
    const totalOrders = visibleOrders.length;
    document.getElementById("showTotalOrders").innerHTML = `Total Escalated Orders - ${totalOrders}`;

    sortOrdersByOrderNoDesc(visibleOrders);
    renderTableRows(currentPage); // uses visibleOrders
    createPaginationControls(Math.ceil(lastFilteredCount / rowsPerPage)); // UPDATED to use rendered count
  } catch (error) {
    console.error("Error fetching current month's orders:", error);
  }

  // === SEARCH (UPDATED to update count via renderTableRows) ===
  $("#searchInput").on("keyup", function () {
    const value = $(this).val().toLowerCase();

    // Filter over allOrders (or you can choose to filter over current visibleOrders after a date filter)
    const base = allOrders; // keep search global; after a date filter, allOrders already limited to that range
    const searched = base.filter(order => {
      return (
        (order.orderDate && String(order.orderDate).toLowerCase().includes(value)) ||
        (order.orderNo && String(order.orderNo).toLowerCase().includes(value)) ||
    ...
    // UPDATED: set visibleOrders to the search results (or reset if empty search)
    visibleOrders = (value === "") ? allOrders.slice() : searched.slice();

    currentPage = 1;
    if (visibleOrders.length > 0 || value === "") {
      renderTableRows(currentPage); // will also update #showTotalOrders
      createPaginationControls(Math.ceil(lastFilteredCount / rowsPerPage));
    } else {
      $('#infoTable').empty().append(`<tr><td colspan="11">No matching results found</td></tr>`);
      // keep header consistent with 0 results
      const headerEl = document.getElementById("showTotalOrders");
      if (headerEl) headerEl.textContent = `Total Escalated Orders - 0`;
      lastFilteredCount = 0;
  ...
  function sortOrdersByOrderNoDesc(orders) {
    return orders.sort((a, b) => {
      const orderNoA = parseInt(String(a.orderNo).replace(/\D/g, ""), 10);
      const orderNoB = parseInt(String(b.orderNo).replace(/\D/g, ""), 10);
  ...
  $("#filterButton").click(async function () {
    ...
    try {
      ...
      // UPDATED: reset visibleOrders to newly filtered dataset
      visibleOrders = allOrders.slice();

      currentPage = 1;
      renderTableRows(currentPage); // updates #showTotalOrders internally
      createPaginationControls(Math.ceil(lastFilteredCount / rowsPerPage));
