// === Ongoing Escalations page (FULL UPDATED CODE) ===
// Changes:
// 1) Keeps the "Ongoing Escalations" count (#showTotalOrders) in sync on search, filter, paginate, and initial load.
...
$(document).ready(async function () {
  ...
  let allOrders = [];
  let visibleOrders = [];           // UPDATED: what we are showing post search/filter
  const rowsPerPage = 25;
  let currentPage = 1;
  let lastFilteredCount = 0;        // UPDATED: count after table-specific filter
  ...
  function renderTableRows(page, orders = visibleOrders) {
    ...
    // Keep only rows that match statuses + have an escalation ticked
    ...
    // UPDATED: update header count based on what will actually be rendered
    lastFilteredCount = filteredOrders.length;
    const headerEl = document.getElementById("showTotalOrders");
    if (headerEl) headerEl.textContent = `Ongoing Escalations - ${lastFilteredCount}`;
    ...
    ordersForPage.forEach((item) => {
      ...
      $("#infoTable").append(`
        <tr>
          ...
          <td>${(item.fName && item.lName) ? `${item.fName} ${item.lName}` : item.customerName || ""}</td>
  ...
  try {
    ...
    // UPDATED: visibleOrders is our working set; render will set the count accurately
  ...
  // Search (UPDATED to keep header & pagination in sync via renderTableRows)
  $("#searchInput").on("keyup", function () {
    const value = $(this).val().toLowerCase();

    const base = allOrders; // search across the currently loaded dataset (post date filter)
    ...
    if (visibleOrders.length > 0 || value === "") {
      ...
      const headerEl = document.getElementById("showTotalOrders");
      if (headerEl) headerEl.textContent = `Ongoing Escalations - 0`;
      lastFilteredCount = 0;
  ...
  // Filter button (UPDATED: header & pagination auto-updated via renderTableRows)
  $("#filterButton").click(async function () {
    ...
    try {
      ...
      // UPDATED: reset working set and re-render
      visibleOrders = allOrders.slice();
  ...
  $("#infoTableHeader th.sortable").on("click", function () {
    ...
    // UPDATED: sort the currently visible dataset
